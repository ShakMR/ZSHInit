...
lastBranches () {
  local branch
  branch=$(git for-each-ref --sort=-committerdate refs/heads/ --format='%(refname:short)' | grep -v "main" | grep -v "master" | head -n 10 | fzf)
  if [[ -n "$branch" ]]; then
    git checkout "$branch"
  fi
}
...
source ${HOME}/.customZsh/zshrc/gitcount.sh